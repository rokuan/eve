...
object TaskPool {
  ...
  def scheduleDelayedTask(action: ActionType, values: List[ObjectValueSource], time: ITimeObject) = {
    time match {
      case v: VerbalGroup =>
  ...
  def scheduleDelayedTask(action: ActionType, values: List[ObjectValueSource], date: Date) = {
  ...
  def scheduleDelayedTask(action: IAction, values: List[ValueSource], when: ITimeObject, how: IWayObject) = {
    ...
    val task = new TimerTask {
      override def run(): Unit = values.foreach { v =>
        ...
        World.findReceiver(interpretationSource).map(_.handleMessage(EveObjectMessage(action, v, howValue)))
      }
    }
